...
class RelayGestureControl:
    ...
    def light_simulation(self, img):
        ...
        # Ghép thanh trạng thái đèn vào dưới cùng ảnh
    ...
    def run(self):
        ...
        try:
            while cam.isOpened():
                ...
                stable_gesture = None
                # Sửa: kiểm tra hand khác None trước khi dùng len
                if hand is not None and len(hand) > 0:
                    ...
                    self.status_text = None
                    self.gesture_buffer.clear()
                # Chỉ thực hiện hành động khi gesture đã ổn định
                if stable_gesture:
                ...
                # UI: show gesture status (giữ tiếng Anh)
                if self.status_text:
